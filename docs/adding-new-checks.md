...
* Place your check in a folder which corresponds with what it does. For example, if it
...
* Name the file of your check based what the code ends up looking like. For example, FURB124
...
expressions, and it is named as such.
  ...
  * One exception is when your check is detecting that you *shouldn't* do something,
  in which case you might prefer to prefix it with `no_`, such as `no_del.py`, which checks
  for usage of the `del` statement.

Once your file has been auto-generated, you will need to increment the error code id (it
defaults to `999`). The easiest way to find the next number is to look for the next number
in the `test/data/` folder. The files named `err_999.py` and `err_999.txt` are the test data
for each check, more on that later. You can also run this shell command to find the next
number for you:
...
In this case, the next number is 132.

Also, if you are making a check for Refurb itself, remove the `suffix` line, which defaults
to `XYZ`. The default is `FURB`, which is used for the built-in Refurb checks.
...
If you where to run the following code, you would get `False`, due to how floating point numbers
work (see [0.30000000000000004.com](https://0.30000000000000004.com/)).
...
This is the AST representation of our code. Some things to note:

* Files are represented by a `MypyFile`.
* A `MypyFile` is composed of a bunch of statements, in this case, `AssignmentStmt`.
* Each `AssignmentStmt` is composed of 3 lines:
  * The `NameExpr`, which is the name/variable being assigned to
  * The expression being assigned (more on that later)
  * Optionally, the type (`builtins.float` in this case)

For our check, we only need to care about the `ComparisonExpr` part.

Now that we have a general idea of what we need to do, lets write some code!

## Writing the Check

We will start by updating the `check` function of our `no_float_cmp.py` file to the following:
...
This code will basically emit an error whenever a comparison expr is hit, regardless of whether
it is a `float` or not.
...
Now lets add some test code to the `tmp.py` file to test that we are only checking for code
that we care about:
...
Notice how I switched the `z` variable to `_`. This is because `_` is a placeholder, and will
gobble up any value you put into it. Since we cannot just write `(x + y) == 0.3` on a line all
...
Now we can translate this logic to code:
...
One issue: The following code will not be checked:
...
of tuples which we can very easily. For example, `1 < 2 == 3` would be converted into:
...
In our `check` function, we:

1. Check if the operator is `==`.
2. If it is, loop through each of the expressions (the left and right hand side of the comparison, that is).
...
Our check works, but it could be simplified. Foe example, we know `node` is a `ComparisonExpr`, and all
we are doing is pulling out the `operators` and `operands` fields, which we know exist on `node`. We could
re-write it like so:
...
```python
    msg: str = "Don't use compare float values with `==`"
```

## The Final Code

Here is the complete code for the final check:
...
```
$ refurb tmp.py --quiet
tmp.py:6:10 [FURB132]: Don't use compare float values with `==`
tmp.py:7:16 [FURB132]: Don't use compare float values with `==`
tmp.py:8:15 [FURB132]: Don't use compare float values with `==`
```

## Testing

This should be pretty because we have been testing as we go! All we need to do now is copy our
